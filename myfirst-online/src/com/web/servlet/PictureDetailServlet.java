 ...
 * @Version 1.0
 **/
@WebServlet(urlPatterns = "/picture_detail/*")
public class PictureDetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        List<Picture> pictureList = (List<Picture>) sc.getAttribute("pictureList");
        //遍历，查找指定id的图书
        for (Picture picture : pictureList) {
            if (Integer.parseInt(id) == picture.getId()) {
                //找到匹配的id，存入request，页面转发到图书详情页，地址栏不变
                req.setAttribute("picture", picture);
//                resp.setContentType("text/plain;charset=utf-8");
//                resp.getWriter().print(book);
                req.getRequestDispatcher("/picture_detail.jsp").forward(req, resp);
            }
        }
    }
}
